    ...
    if ($objSesion->verificar_sesion_si_activa($id_sesion, $token)) {
        ...
        if (!empty($arr_Ambiente)) {
            ...
            for ($i = 0; $i < count($arr_Ambiente); $i++) {
                ...
                <a href="'.BASE_URL.'imprimir-movimiento/'.$arr_Ambiente[$i]->id.'"class="btn btn-outline-secondary"><i class="fa fa-print"></i></a>
                ';
...
if ($tipo == 'buscar_movimiento_id'){
    ...
    $arrAmbIes = $objInstitucion->buscarInstitucionById($arrMovimiento->id_ies);
